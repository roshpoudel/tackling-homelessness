...
file_path = os.path.join(os.path.dirname(
    __file__), '..', 'utils', 'columns_mapping.json')
...
class lot_homeless_graphs:
    ...
    df_lot = df.groupby(['Demographic Grouping', 'lot_homeless']).count()[
        'age'].reset_index(name='counts')
    df_lot['Total'] = df_lot.groupby('Demographic Grouping')[
    ...
    def lot_homeless_graph2(cls) -> go.Figure:
        ...
        fig = px.bar(lot_8, x="Demographic Grouping", y="Percentage",
                     title="Distribution of 8+ years in LOT Homeless by race and gender")
    ...
    def lot_homeless_graph3(cls) -> go.Figure:
        ...
        fig = px.bar(lot_12, x="Demographic Grouping", y="Percentage",
                     title="Distribution of under 12 months in LOT Homeless by race and gender")
        fig.update_layout(bargap=0.5, title_x=0.5)
    ...
    def lot_homeless_graph4() -> go.Figure:
        df_lot_score = df.groupby(['Demographic Grouping', 'LOT_homeless_score']).count()[
            'age'].reset_index(name='counts')
        df_lot_score['Total'] = df_lot_score.groupby('Demographic Grouping')[
            'counts'].transform('sum')
        df_lot_score['Percentage'] = (
            df_lot_score['counts'] / df_lot_score['Total']) * 100
        df_lot_score['Percentage'] = df_lot_score['Percentage'].round(2)
        value_to_filter = 1
        df_filtered = df_lot_score[df_lot_score['LOT_homeless_score'].eq(
            value_to_filter)]

        fig = px.bar(df_filtered, x="Demographic Grouping", y="Percentage",
                     title="Distribution of score 1 in LOT Homeless by race and gender")
        fig.update_layout(bargap=0.5, title_x=0.5)
...
class health_scale_graphs():
    ...
    df_health = df.groupby(['Demographic Grouping', 'health_scale']).count()[
        'age'].reset_index(name='counts')
    df_health['Total'] = df_health.groupby('Demographic Grouping')[
        'counts'].transform('sum')
    ...
    def health_scale_graph2(cls) -> go.Figure:
        ...
        df_health_poor = cls.df_health.loc[cls.df_health['health_scale']
                                           == value_to_filter1]
        fig = px.bar(df_health_poor, x="Demographic Grouping", y="Percentage",
                     color='health_scale', title="Distribution of Reported Poor Health by race and gender")
        fig.update_layout(bargap=0.5, title_x=0.5)
        fig.update_traces(marker_color='#1f77b4')
        return fig
...
class risks_and_barriers_graphs():
    ...
    def helper(col_name: str) -> pd.DataFrame:
        df_risks = df.groupby(['Demographic Grouping', col_name]).count()[
            'age'].reset_index(name='counts')
        df_risks['Total'] = df_risks.groupby('Demographic Grouping')[
            'counts'].transform('sum')
    ...
    def risks_and_barriers_graph1() -> go.Figure:
        fig = px.box(df, x='Demographic Grouping',
                     y="risks_and_barriers_score")
        return fig
    ...
    def risks_and_barriers_graph2(cls) -> go.Figure:
        ...
        fig = px.bar(df_d, x="Demographic Grouping", y="Percentage", color=col_name,
                     title="Distribution of "+f'{col_name}'+" by race and gender")
    ...
    def risks_and_barriers_graph3(cls) -> go.Figure:
        ...
        domestic_df = df_dv1.merge(
            df_dv, how='inner', on='Demographic Grouping')

        domestic_df["Experienced Domestic Violence"] = domestic_df['counts_x'] + \
            domestic_df["counts_y"]
        domestic_df["Percent Experienced DV"] = domestic_df['Percentage_x'] + \
            domestic_df["Percentage_y"]

        fig = px.bar(domestic_df, x="Demographic Grouping",
                     y="Percent Experienced DV", title="Experienced Domestic Violence")
        fig.update_layout(bargap=0.5, title_x=0.5)
    ...
    def risks_and_barriers_graph4() -> go.Figure:
        ...
        category_count_df = df[risksbarriers].applymap(
            lambda cell: cell == category_to_count)
        ...
        skip_df = pd.DataFrame({'Question': column_counts.index,
                               'Count': column_counts.values, 'Percentage': column_percentages.values})

        fig = px.bar(skip_df, x="Question", y="Percentage",
                     title="Skips in Risks and Barriers")
        fig.update_layout(bargap=0.5, title_x=0.5)
...
class household_type_graphs:
    ...
    def household_type_graph2() -> go.Figure:
        ...
        fig = px.bar(df_dv1, x="Demographic Grouping", y="counts",
                     color="fleeing_dv?", title="Fleeing Domestic Violence")
        fig.update_layout(bargap=0.5, title_x=0.5)
        return fig

    @staticmethod
    def household_type_graph3() -> go.Figure:
        household_scores_count = df['household_type_score'].value_counts(
        ).reset_index()
        household_scores_count.columns = ['Household Type Score', 'Count']
        household_scores_count.drop([0, 1, 2, 3, 4, 6, 8], inplace=True)
        fig = px.bar(household_scores_count, x='Household Type Score', y='Count',
                     title='Count of scores indicating an individual is fleeing domestic violence')
        return fig
    ...
    def household_type_graph4() -> go.Figure:
        ...
        fig = px.bar(df_house_score, x="Demographic Grouping", y="counts", color=colm_name,
                     title="Distribution of Household Type Score by race and gender")
        fig.update_layout(bargap=0.5, title_x=0.5)
        return fig
    ...
    def household_type_graph5() -> go.Figure:
        colm_name = 'household_type_score'
        df_house_score = risks_and_barriers_graphs.helper(colm_name)
        fig = px.bar(df_house_score, x="Demographic Grouping", y="Percentage",
                     color=colm_name, title="Distribution of Household Type Score by race and gender")
        fig.update_layout(bargap=0.5, title_x=0.5)
        return fig
    ...
    def household_type_graph6() -> go.Figure:
        ...
        fig = px.bar(df_housesize, x="Demographic Grouping", y="counts",
                     color=colm_name, title="Distribution of Household Size by race and gender")
        fig.update_layout(bargap=0.5, title_x=0.5)
        return fig
    ...
    def household_type_graph7() -> go.Figure:
        colm_name = 'household_size'
        df_housesize = risks_and_barriers_graphs.helper(colm_name)
        fig = px.bar(df_housesize, x="Demographic Grouping", y="Percentage",
                     color=colm_name, title="Distribution of Household Size by race and gender")
        fig.update_layout(bargap=0.5, title_x=0.5)
        return fig
...
class living_situation_graphs():
    ...
    def living_situation_graph2() -> go.Figure:
        ...
        index_names = df_ls[df_ls['living_situation']
                            == 'Doubled up / couch surfing'].index
        df_ls.drop(index_names, inplace=True)
        index_names = df_ls[df_ls['living_situation'] ==
                            'Hotel or temporary rental requiring ongoing payment'].index
        df_ls.drop(index_names, inplace=True)
        index_names = df_ls[df_ls['living_situation'] ==
                            'Own home, including rental and family housing'].index
        df_ls.drop(index_names, inplace=True)
        index_names = df_ls[df_ls['living_situation'] ==
                            'Medical or behavioral health inpatient care'].index
        df_ls.drop(index_names, inplace=True)

        fig = px.bar(df_ls, x="Demographic Grouping", y="Percentage", color=colm_name,
                     title="Distribution of Literal Homelessness by race and gender", color_discrete_sequence=["blue", "goldenrod", "magenta"])
        fig.update_layout(bargap=0.5, title_x=0.5)
        return fig
    ...
    def living_situation_graph3() -> go.Figure:
        ...
        fig = px.bar(df_ls_score, x="Demographic Grouping", y="Percentage", color=colm_name,
                     title="Distribution of Literal Homelessness Scoring by race and gender", color_discrete_sequence=["blue", "goldenrod", "magenta"])
